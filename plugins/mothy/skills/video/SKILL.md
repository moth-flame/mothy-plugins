...
- **Playwright `recordVideo`** (headless is fine; retry headed only if charts/canvas don't paint). `recordVideo:{dir,size:{1920,1080}}`. One context per beat = one webm. NOT avfoundation (perm-gated, leaks desktop, non-deterministic). NOT screenshot-frames (stutters).
